...
<!-- RiGHT -->
                <div class="col-9 right-message">
                    <div class="container shell-message">
                        ...
                        <?php
                        if (empty($messageAndUsers)):
                        ?>
                        <h5 class="h5">У Вас пока нет сообщений</h5>
                        <?php
                        else:
                        $preserved_message = array_reverse($messageAndUsers, true);
                        foreach ($preserved_message as $key => $message):
                            if ($message['id_user_rec'] == 0):
                                ?>
                                <div class="row row-mes">
                                    <div class="col-3">
                                        <?= mb_strimwidth($message['theme'],0,25, '...') ?>
                                    </div>
                                    <div class="col-5">
                                    ...
                                    <div class="col-2 for-link">
                                        <button onclick="location.href = '<?php echo BASE_URL . 'admin/admin_message_feedback_check.php?id=' . $message['id_message']?>';" name="but-check" type="submit" class="btn btn-success but-check">Прочитать</button>
                                    </div>
